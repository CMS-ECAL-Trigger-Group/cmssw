  ...
  void testWriteOddWeights() {
    ...
    // this is an example for writing the odd weights
      ...
      for (int ich=0; ich<(int) EB_ecid_vec.size() ; ich++){
        ...
        std::cout << "EB "<< EB_ecid_vec[ich].getID1() << "  "<< EB_ecid_vec[ich].getID2() << "  "<< EB_ecid_vec[ich].getID3() << std::endl;
      ...
      for (int ich=0; ich<(int) EE_ecid_vec.size() ; ich++){
        FEConfigOddWeightDat weid;
        if (EE_ecid_vec[ich].getID1()>609 && EE_ecid_vec[ich].getID1()<=645) continue;
        std::cout << "EE "<< EE_ecid_vec[ich].getID1() << "  "<< EE_ecid_vec[ich].getID2() << "  "<< EE_ecid_vec[ich].getID3() << std::endl;
